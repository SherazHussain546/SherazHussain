...
export default function CertificationsSection() {
  const certsCollection = firestore ? collection(firestore, 'certifications') : null;
  const certsQuery = certsCollection ? query(certsCollection, orderBy('createdAt', 'desc')) : null;
  const [snapshot] = useCollection(certsQuery);

  const dynamicCerts = snapshot?.docs.map(doc => ({ id: doc.id, ...doc.data() } as Certification)) || [];

  const allCerts = useMemo(() => {
    const formattedStatic = staticCerts.map((c, i) => ({ ...c, id: `static-${i}`, iconName: c.icon.name || 'Award' }));
